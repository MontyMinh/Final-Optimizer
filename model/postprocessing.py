...
def postprocess():
    ...
    # Unpack Cost (Convert to USD 1:23k)
    Results.cost.append(
        (Data.linear_program * Data.objective_vector)[:, np.newaxis]/23_000)
...
def save_to_excel():
    ...
    with pd.ExcelWriter(Results.save_location) as writer:
        ...
        # Product List
        df = pd.DataFrame(data=Data.product_list, columns=['PRODUCT'])
        quick_save(df, "Product List")
